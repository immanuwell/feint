//! End-to-end smoke test: runs the actual compiled `seedy` binary against
...
async fn init_then_up_smoke_test() {
    ...
    let config_path = std::env::temp_dir().join(format!("seedy_smoke_{}.yaml", std::process::id()));

    let init_output = Command::cargo_bin("seedy")
        .unwrap()
        .args(["init", &url, "--config"])
        .arg(&config_path)
        .output()
        .expect("run seedy init");
    assert!(
    ...
    assert!(
        config_path.exists(),
        "seedy init did not write {config_path:?}"
    );

    let up_output = Command::cargo_bin("seedy")
        .unwrap()
        .args(["up", &url, "--config"])
        .arg(&config_path)
        .output()
