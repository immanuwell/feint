...
use seedy_core::graph::{self, InsertGroup, TreeNode};
use seedy_core::introspect;
...
pub async fn run(database_url: &str, config_path: &Path, schemas: &[String]) -> anyhow::Result<()> {
    ...
    // A seedy.yaml may not exist yet — `plan` is useful both before and
    // after `init`, falling back to default row counts when there's none.
    let config = SeedyConfig::load(config_path).ok();
            ...
            for group in &plan.groups {
                ...
                for table in group.tables() {
                    let rows = config
                        ...
                        .map(|tc| tc.rows)
                        .unwrap_or(seedy_core::config::DEFAULT_ROWS);
    ...
    let sensitive_count: usize = schema
        ...
        .filter(|c| seedy_core::generate::classify_sensitive(&c.name).is_some())
        .count();
    if sensitive_count > 0 {
        println!("Sensitive columns: {sensitive_count} (see `seedy init` for details)");
    }
