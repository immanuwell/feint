...
//! optionally, custom generators in `seed.ts`) to a `seedy.yaml`.
//!
...
use crate::config::{SeedyConfig, TableConfig, DEFAULT_ROWS, DEFAULT_SEED};
...
/// source) into a `SeedyConfig` plus a report of what needs manual review.
    ...
    if entries.is_empty() {
        report.general_notes.push(
            "No `select` array found in seed.config.ts (or it wasn't in a form this parser \
             recognizes). Snaplet Seed selects every table by default in that case — run `seedy \
        ...
        match entry {
            ...
            SelectEntry::Include(name) => {
                report.general_notes.push(format!(
                    "select includes the glob pattern `{name}` — seedy.yaml needs literal table \
                     names. Run `seedy init <database-url>` and keep the tables matching this \
            ...
            SelectEntry::Exclude(name) => {
                report.general_notes.push(format!(
                    "select excludes `{name}` — make sure this table isn't listed in the \
                     generated seedy.yaml, or remove it by hand after `seedy init`."
                ));
            }
        }
    }
        ...
        for table in detect_custom_generators(seed_ts) {
            report.skipped.push(SkippedColumn {
                ...
                reason: "seed.ts defines custom per-row generator logic for this model in \
                         ...
                         add explicit `generator:` overrides in seedy.yaml where you still want \
...
/// schema-qualified (`"public.users"`); seedy.yaml always wants
/// `schema.table`.
