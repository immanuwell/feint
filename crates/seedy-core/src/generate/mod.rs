...
/// `seedy.yaml` override, a column-name heuristic, then a column-type
/// heuristic.
    ...
    match type_kind {
        ...
        TypeKind::Composite | TypeKind::Other => {
            if nullable {
                ...
                Err(SeedyError::UnsupportedType(format!(
                    "type `{type_name}` has no built-in generator; add an explicit `generator:` override in seedy.yaml for this column"
                )))
            }
        }
...
fn dispatch_named(name: &str, column: &Column, rng: &mut ChaCha8Rng) -> Result<PgValue> {
    Ok(match name {
        ...
        other => {
            return Err(SeedyError::Generation(format!(
