...
//!
//! Runs the whole `seedy up` inside a single transaction: any failure
...
fn rows_for(config: &SeedyConfig, table_id: &TableId) -> u32 {
    config
                ...
                if all_nullable {
                    ...
                    return Err(SeedyError::Config(format!(
                        "table `{}` needs at least one row in `{}` to satisfy foreign key `{}`, but none exist \
                         (increase its `rows:` in seedy.yaml, or configure this table after it)",
                        table.id.qualified(),
...
async fn insert_backfill_group(
    ...
    for table_id in tables {
        ...
        let pk_cols = table
            .primary_key
            .clone()
            .ok_or_else(|| SeedyError::Config(format!(
...
async fn insert_deferred_group(
    ...
    // seedy still has to know the *value* to write at insert time. That's
                    ...
                    if col.is_server_assigned() {
                        return Err(SeedyError::Config(format!(
                            "table `{}` has a deferrable foreign-key cycle through `{}`, but `{}`.`{}` is a \
                             server-assigned (serial/identity) column seedy can't pre-generate a value for. \
