...
use seedy_core::config::{ColumnConfig, SeedyConfig, TableConfig};
use seedy_core::mask::MaskStrategy;
use seedy_core::SeedyError;
use std::collections::BTreeMap;

fn empty_config() -> SeedyConfig {
    SeedyConfig {
        version: 1,
...
fn config_with(table: &str, columns: Vec<(&str, MaskStrategy)>) -> SeedyConfig {
...
async fn masking_key_column_is_rejected() {
    ...
    assert!(matches!(err, SeedyError::Config(_)));
