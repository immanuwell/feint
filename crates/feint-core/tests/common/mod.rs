...
async fn shared_port() -> u16 {
    ...
    let container = CONTAINER
        .get_or_init(|| async {
            ...
            // catalog columns feint relies on (e.g. `pg_attribute
...
impl TestDb {
    ...
    pub async fn introspect(&self) -> Schema {
        feint_core::introspect::introspect(&self.client, std::slice::from_ref(&self.schema_name))
            .await
            .expect("introspect")
    }
...
fn feint_test_config(schema: &Schema, rows: u32) -> FeintConfig {
...
impl CloneFixture {
    ...
    pub async fn introspect_source(&self) -> Schema {
        feint_core::introspect::introspect(
            &self.source_client,
    ...
    /// Run `feint_core::clone::run` end to end: read-only source
    ...
    pub async fn clone(
        ...
        schema: &Schema,
        config: &FeintConfig,
    ) -> feint_core::Result<feint_core::clone::CloneSummary> {
    ...
    pub async fn clone_subset(
        ...
        let subset = match root {
            Some(spec) => {
                let parsed = feint_core::subset::parse_root(schema, spec)?;
                let rows = feint_core::subset::compute_subset(
                    &source_txn,
                    schema,
                    &parsed,
                    &feint_core::subset::SubsetOptions::default(),
                )
                .await?;
                Some(rows)
            }
        ...
        let result = feint_core::clone::run(
            &source_txn,
            &target_txn,
            schema,
            &plan,
