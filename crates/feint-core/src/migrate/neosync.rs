//! Best-effort migration from a Neosync Job export to `feint.yaml`.
//!
...
use crate::config::{ColumnConfig, FeintConfig, TableConfig, DEFAULT_ROWS, DEFAULT_SEED};
...
/// into `feint.yaml` masking overrides.
...
/// was set, e.g. `generateEmailConfig`) to the closest feint masking
...
fn map_transformer(config_key: &str) -> Mapped {
    match config_key {
        // Exact matches: feint has the identical concept.
        ...
        // Exact matches: feint has a purpose-built generator for this shape.
        ...
        // Approximate: closest feint strategy, but the semantics differ
        ...
        "nullconfig" | "nullConfig" => Mapped::Approximate {
            ...
            note: "Neosync's Null transformer always writes NULL (and fails outright on a NOT \
                   NULL column there too); feint's redact adapts, writing NULL on a nullable \
        ...
        | "generateUnixTimestampConfig" => Mapped::Approximate {
            mask: MaskStrategy::Fake,
            generator: None,
            note: "feint has no purpose-built generator matching this Neosync transformer yet; \
                   falls back to its generic type-based generator, which won't look like this \
                   specific data shape.",
        },

        // Unsupported: no feint equivalent, or arbitrary code that can't
        // be mechanically converted.
        "generateDefaultConfig" => Mapped::Unsupported {
            note: "uses the column's own DEFAULT expression, which feint has no equivalent for. \
